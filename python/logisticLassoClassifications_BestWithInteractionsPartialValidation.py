...
# make all binary input features -1 or 1
...
for tb in toBinary:
    df[tb]  = 2*df[tb].map(dict(YES=1, NO=0)) - 1
...
# log transform to make range/distribution more compact
#df['PSA'] = np.log(df['PSA'])
#df['ProstateVolume'] = np.log(df['ProstateVolume'])

# hand-picked interactions
...
logisticScaledModel = {"model": Pipeline(steps=[('scaler', StandardScaler()), ('logistic', LogisticRegression(solver="liblinear", max_iter=10000, penalty='l1'))]),
                 "name": "Logistic",
                 "p_grid": {"logistic__C": np.logspace(-2,1,20)},
                 "scoring": "neg_log_loss", "result": {}}

randomForestModel = {"model": RandomForestClassifier(criterion='gini', n_estimators=100, bootstrap=True),
                     "name": "Random Forest",
                     "p_grid": {"max_depth": [2, 6, 12]}, #, 8, 10]},
                     "scoring": scoring}
...
for estimator in [logisticScaledModel]: #randomForestModel]: #svmModel]: # # # # #]: #univariateModel]: #]: #,]: #]: #
    ...
    if estimator is not univariateModel and estimator is not svmModel:
        ...
        elif estimator is logisticScaledModel:
            values = np.squeeze(clfAll.best_estimator_.steps[1][1].coef_)
    ...
    print('AUCROC   (CV)    = \033[1m' + str(np.mean(scores_roc_auc).round(3)) + '\033[0m' + ' \u00B1 ' + str(np.std(scores_roc_auc).round(3)))
    ...
    for n, res in enumerate(cv_result['estimator']):
        if estimator is logisticModel:
            ...
            coef[n,:] = res.best_estimator_.steps[1][1].coef_
    coef_df = pd.DataFrame(data=coef, columns=featureNames)

    import seaborn as sns

    ax = plt.plot([0, 0], [-0.5, 10.5], color='k')
    # for x in range(10):
    #      plt.plot([x, x],[-10, 10], color='k', linewidth=1.5)
    ax = sns.violinplot(data=coef_df, scale='width', orient='h')
    ax.set_xticklabels(labels=ax.get_xticklabels(), horizontalalignment='left')
    ax.set_xlim((-1.5, 2.5))
    plt.show()
