...
# groupSelectorList = ['MeshVolume', 'shape', 'firstorder|histogram', textureStr, '']
groupSelectorList = ['']
...
# LR full pipeline including unsupervised steps followed by steps with parameter tuning
...
CV_estimator_LR = GridSearchCV(estimator=groupSelect_LR['pipeline'], param_grid=groupSelect_LR['param_grid'], cv=inner_cv, refit=True, verbose=0, scoring=groupSelect_LR['scoring'], n_jobs=n_jobs)
...
pipeline_LR = Pipeline(steps=[
                              ...
                              ('CV_estimator', CV_estimator_LR)
                             ])

# SVM full pipeline including unsupervised steps followed by steps with parameter tuning
...
CV_estimator_SVM = GridSearchCV(estimator=groupSelect_SVM['pipeline'], param_grid=groupSelect_SVM['param_grid'], cv=inner_cv, refit=True, verbose=0, scoring=groupSelect_SVM['scoring'], n_jobs=n_jobs)

pipeline_SVM = Pipeline(steps=[
                               ('scaler', StandardScalerDf()),
                               ('correlationSelector', featureSelection_correlation(threshold=0.9, exact=True, featureGroupHierarchy=featureGroupHierarchy)),
                               ('CV_estimator', CV_estimator_SVM)
                              ])

# RF full pipeline including unsupervised steps followed by steps with parameter tuning
...
CV_estimator_RF = GridSearchCV(estimator=groupSelect_RF['pipeline'], param_grid=groupSelect_RF['param_grid'], cv=inner_cv, refit=True, verbose=0, scoring=groupSelect_RF['scoring'], n_jobs=n_jobs)

pipeline_RF = Pipeline(steps=[
                               ('scaler', StandardScalerDf()),
                               ('correlationSelector', featureSelection_correlation(threshold=0.9, exact=True, featureGroupHierarchy=featureGroupHierarchy)),
                               ('CV_estimator', CV_estimator_RF)
                              ])

# CV_estimator_LR is dummy
modelSelector_pipeline = Pipeline([('classifier', CV_estimator_LR)])

modelSelector_params = [{'classifier':[CV_estimator_LR]}, {'classifier':[CV_estimator_SVM]}, {'classifier':[CV_estimator_RF]}]
...
modelSelector = GridSearchCV(modelSelector_pipeline, modelSelector_params, cv=3, n_jobs=-1, scoring='roc_auc')
...
print(pipeline.steps[2][1].best_estimator_.steps[0][1].best_estimator_.steps[1][0])
#
...
for est in cv_result['estimator']:
    thisModel = est.steps[2][1].best_estimator_.steps[0][1].best_estimator_.steps[1][0]
