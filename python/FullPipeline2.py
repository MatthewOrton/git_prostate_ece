...
sys.path.append('/Users/hwang01/ECE_Semantic_Data/Radiomics')
...
df = pd.read_csv("/Users/hwang01/ECE_Semantic_Data/discovery.csv")
zradata = '/Users/hwang01/ECE_Semantic_Data/Radiomics/extractions/Repro_Z_radiomicFeatures__202205271328/radiomicFeatures/radiomicFeatures.csv'
...
def featureSelection_boruta(X, y):
    fs = BorutaPy4CV(n_estimators=1000, perc=100, alpha=0.05,two_step=True, max_iter=100, random_state=None, verbose=0,weak=True)
    fs.fit(X.values, y)
    X_filtered = X.loc[:, fs.support_]
    return X_filtered
...
def makeFitAndCV(X, y, borutaPassThrough=False, n_splits_inner=5, n_splits_outer=5, n_repeats=10, verbose=0):
    ...
    LR_param = {'classifier__C': np.logspace(-2, 0, 20),
    ...
    allSteps = [('scaler', StandardScalerDf()),
                ...
                ('borutaOuter', borutamodelSelector),
               ]
...
for borutaPassThrough in [True]:
    ...
    pipeline, cv_result, modelDict = makeFitAndCV(X, y, borutaPassThrough=borutaPassThrough, n_splits_inner=10, n_splits_outer=5, n_repeats=100)
