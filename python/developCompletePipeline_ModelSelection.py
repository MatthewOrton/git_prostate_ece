...
# pipeline_LR is dummy
pipeline = Pipeline([('classifier', pipeline_LR)])

params = [{'classifier':[pipeline_LR]}, {'classifier':[pipeline_SVM]}, {'classifier':[pipeline_RF]}]
...
gs = GridSearchCV(pipeline, params, cv=3, n_jobs=-1, scoring='roc_auc')

gs.fit(X, y)

print(gs.best_params_['classifier'].steps[2][1].estimator.steps[1][0])

outer_cv = RepeatedStratifiedKFold(n_splits=n_splits_outer, n_repeats=n_repeats)
# outer_cv = LeaveOneOut()
# outer_cv = LeavePairOut()
cv_result = cross_validate(gs, X=X, y=y, cv=outer_cv, scoring=scoringList, return_estimator=True, verbose=verbose)
...
for est in cv_result['estimator']:
    thisModel = est.best_params_['classifier'].steps[2][1].estimator.steps[1][0]
