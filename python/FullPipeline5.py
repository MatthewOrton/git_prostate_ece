...
def makeFitAndCV(X, y, n_splits_inner=5, n_splits_outer=5, n_repeats=10, verbose=0):
    ...
    LR_param = {'classifier__C': np.logspace(-1, 3, 20),
                'classifier': [LR_model]}

    SVM_model = SVC(kernel='rbf')
    SVM_param = {'classifier__C': np.logspace(3, 9, 8),
                 'classifier__gamma': np.logspace(-10, -4, 8),
                 'classifier': [SVM_model]}

    RF_model = RandomForestClassifier(max_depth=10, min_samples_leaf=4)
    RF_param = {'classifier__max_features': [2, 3, 4, 5, 6, 8],
                'classifier': [RF_model]}
    ...
    modelSelector_params = [SVM_param] #, LR_param, RF_param]
    ...
    pipeline = Pipeline(allSteps)

    pipelineTest = Pipeline(steps=[('scaler', StandardScalerDf()),
                ('correlationSelector', featureSelection_correlation(threshold=0.9, exact=True, featureGroupHierarchy=featureGroupHierarchy, outputNumpy=True)),
                ('boruta', BorutaPy4CV(n_estimators='auto', perc=100, alpha=0.05, two_step=True, max_iter=100, verbose=0, weak=True)),
                ('model', SVC(kernel='rbf'))])

    start_time = time.time()
    pipelineTest.fit(X,y)
    ...
    # print some results
    bestModel = pipeline.steps[2][1].best_estimator_._final_estimator
    print(modelDict[bestModel.__class__])

    print('# features in        = ' + str(len(pipeline.steps[1][1].mask_)))
    print('# features after CFR = ' + str(np.sum(pipeline.steps[1][1].mask_)))
...
for borutaPassThrough in [True]:
    ...
    pipeline, cv_result, modelDict = makeFitAndCV(X, y, n_splits_inner=10, n_splits_outer=10, n_repeats=5)
    ...
    for est in cv_result['estimator']:
        thisModel = modelDict[est.steps[2][1].best_estimator_.steps[0][1].__class__]
        if thisModel in modelCount:
            modelCount[thisModel] += 1
        else:
            modelCount[thisModel] = 1
